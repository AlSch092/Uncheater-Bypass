...
void Hooks::WriteAbsoluteJump64(void* absJumpMemory, void* addrToJumpTo) //using this hook method we need to pop r10 as the first instruction of our asm
{
...
BOOL Hooks::hk_SetSendBuffer(BYTE* data, INT Length) //our 'callback'
{
	PacketHelper::CreatePacket(data, Length);
	return TRUE;
}

VOID Hooks::ClientDebugLogCallback(LPVOID ptrMessage)
{
	if (ptrMessage != NULL)
	{
		wchar_t* msg = DereferenceSafe<wchar_t*>((UINT64)ptrMessage);
		if (msg != NULL)
			wprintf(L"%s\n", msg);
	}
